...
const injectPTTMocks = () => {
  ...
  class FakeMediaRecorder {
    ...
    public ondataavailable: ((event: { data: Blob; size: number }) => void) | null;
...
async function performLogin(page: Page, username: string): Promise<AuthenticatedUser> {
  ...
  try {
    await page.waitForURL('**/realms/botct/protocol/openid-connect/auth**', { timeout: 10000 });
  } catch {
    // If not redirected to Keycloak, look for login button
    await page.waitForSelector('button:has-text("Login")', { timeout: 15000 });
    await page.click('button:has-text("Login")');
    await page.waitForURL('**/realms/botct/protocol/openid-connect/auth**', { timeout: 30000 });
  }
  ...
  const [response] = await Promise.all([
    page.waitForResponse(response => response.url().includes('login-actions/authenticate'), { timeout: 30000 }),
    submitButton.click()
  ]);
  ...
  try {
    await page.waitForURL(url => url.toString().includes('localhost:5173') && !url.toString().includes('keycloak'), { timeout: 45000 });
  } catch (error) {
  ...
  await page.waitForLoadState('networkidle', { timeout: 30000 });
  await page.waitForSelector('button:has-text("Create Game")', { timeout: 30000 });
...
export const test = baseTest.extend<TestFixtures>({
  ...
  loginMultipleUsers: async ({ browser }, use) => {
    const loginMultipleUsers = async (count: number, options: MultiUserOptions = {}) => {
      ...
      for (let i = 1; i <= count; i++) {
        ...
        if (options.grantMicrophone) {
          await context.grantPermissions(['microphone'], { origin: 'http://localhost:3001' });
        }
