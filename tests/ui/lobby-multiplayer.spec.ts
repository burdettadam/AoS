...
import type { TestFixtures } from "./fixtures/base.fixture";
...
async function bootstrapLobby(
  ...
  for (const name of [PLAYER_ONE_NAME, PLAYER_TWO_NAME]) {
    await expect(storytellerPage.getByText(name)).toBeVisible({
      timeout: 15000,
    });
  }
...
test.describe("Lobby Multiplayer Scenarios", () => {
  test("players join storyteller lobby and are listed for everyone", async ({
    loginMultipleUsers,
  }) => {
    ...
    try {
      await expect(storytellerPage.locator("text=Players (2)")).toBeVisible({
        timeout: 10000,
      });
  ...
  test("storyteller can delegate the storyteller role to another player", async ({
    loginMultipleUsers,
  }) => {
    const setup = await bootstrapLobby(loginMultipleUsers);
    const { storytellerPage, playerPages, cleanup } = setup;
    ...
    try {
      const targetRow = storytellerPage
        .locator(".card", { hasText: PLAYER_ONE_NAME })
        .first();
      await targetRow.locator('button[title="Make Storyteller"]').click();

      await expect(
        storytellerPage.locator(".card", { hasText: "Storyteller" }),
      ).toContainText(PLAYER_ONE_NAME, { timeout: 15000 });
      await expect(
        playerPages[0].locator(".card", { hasText: "Storyteller" }),
      ).toContainText(PLAYER_ONE_NAME, { timeout: 15000 });
    } finally {
      await cleanup();
    }
  ...
  test("players can propose and vote on shared scripts", async ({
    loginMultipleUsers,
  }) => {
    const setup = await bootstrapLobby(loginMultipleUsers);
    const { storytellerPage, playerPages, cleanup } = setup;
    ...
    try {
      await storytellerPage.waitForSelector('[data-testid^="all-script-"]', {
        timeout: 15000,
      });
      const scriptButton = storytellerPage
        .locator('[data-testid^="all-script-"]')
        .first();
      ...
      await storytellerPage
        .locator(`[data-testid="master-toggle-${scriptId}"]`)
        .click();
      await expect(
        storytellerPage.locator(`[data-testid="shared-script-${scriptId}"]`),
      ).toBeVisible({ timeout: 15000 });

      for (const page of playerPages) {
        await expect(
          page.locator(`[data-testid="shared-script-${scriptId}"]`),
        ).toBeVisible({ timeout: 15000 });
      }

      const proposerToggle = playerOnePage.locator(
        `[data-testid="proposal-toggle-${scriptId}"]`,
      );
      await proposerToggle.click();
      await expect(proposerToggle).toHaveAttribute("aria-pressed", "true", {
        timeout: 10000,
      });

      const playerOneProposalCard = playerOnePage
        .locator('[data-testid^="proposal-card-"]')
        .filter({ hasText: scriptLabel })
        .first();
      await expect(playerOneProposalCard).toBeVisible({ timeout: 15000 });
      await expect(
        playerOneProposalCard.locator("text=You proposed"),
      ).toBeVisible();

      const playerTwoProposalCard = playerTwoPage
        .locator('[data-testid^="proposal-card-"]')
        .filter({ hasText: scriptLabel })
        .first();
      await expect(playerTwoProposalCard).toBeVisible({ timeout: 15000 });

      const storyProposalCard = storytellerPage
        .locator('[data-testid^="proposal-card-"]')
        .filter({ hasText: scriptLabel })
        .first();
      await expect(storyProposalCard).toBeVisible({ timeout: 15000 });

      const playerTwoVoteUp = playerTwoProposalCard
        .locator('[data-testid^="vote-up-"]')
        .first();
      await playerTwoVoteUp.click();
      await expect(playerTwoVoteUp).toHaveAttribute("aria-pressed", "true", {
        timeout: 10000,
      });
      await expect(
        playerTwoProposalCard.locator("text=Popularity score: 1"),
      ).toBeVisible({ timeout: 10000 });

      await expect(
        playerOneProposalCard.locator("text=Popularity score: 1"),
      ).toBeVisible({ timeout: 10000 });
      await expect(
        storyProposalCard.locator("text=Popularity score: 1"),
      ).toBeVisible({ timeout: 10000 });
    } finally {
      await cleanup();
    }
  ...
  test("press-to-talk storyteller speaking suppresses other microphones", async ({
    loginMultipleUsers,
  }) => {
    ...
    try {
      ...
      await expect(storytellerMicButton).toHaveAttribute(
        "title",
        /stop speaking/i,
        { timeout: 5000 },
      );
      ...
      await playerMicButton.click();
      await expect(playerMicButton).toHaveAttribute("title", /stop speaking/i, {
        timeout: 5000,
      });
