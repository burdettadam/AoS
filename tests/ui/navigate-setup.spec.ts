...
test.describe('Game Setup Navigation', () => {
  test('Storyteller navigates to setup page', async ({ page, loginAsUser, createGame }) => {
    ...
    const beginSetupBtn = page.getByRole('button', { name: /Begin Game Setup/i });
    ...
    await expect.poll(async () => page.url(), { timeout: 30000 }).toMatch(/\/setup\//);

    // Allow phase transition to complete (auto enter may be async)
    await page.waitForSelector('[data-testid="game-setup-info-panel"]', { timeout: 30000 });
    ...
    await expect(page.locator('[data-testid="game-setup-info-panel"]')).toBeVisible();
