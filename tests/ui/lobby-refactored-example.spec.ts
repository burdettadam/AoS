...
import { test as originalTest, expect } from '@playwright/test';
import { test, expect as fixtureExpect } from './fixtures';
...
originalTest.describe('Lobby UX (Original)', () => {
  ...
  originalTest('should allow script selection', async ({ page }) => {
    // Test implementation...
    await expect(page.locator('button:has-text("Trouble Brewing")')).toBeVisible();
  });
});
...
test.describe('Lobby UX (With Fixtures)', () => {
  ...
  test('should allow script selection using fixtures', async ({ page, selectScript }) => {
    // Much cleaner test implementation
    await selectScript('Trouble Brewing');
    await expect(page.locator('.script-selected:has-text("Trouble Brewing")')).toBeVisible();
  });
  
  test('should handle multiple script changes', async ({ page, selectScript }) => {
    await selectScript('Trouble Brewing');
    await selectScript('Sects & Violets');
    await selectScript('Bad Moon Rising');
    
    // Verify final selection
    await expect(page.locator('.script-selected:has-text("Bad Moon Rising")')).toBeVisible();
  });
});

// Multi-user scenario example
test.describe('Multi-User Lobby Scenarios', () => {
  test('storyteller and player workflow', async ({ browser, loginAsUser, createGame, goToLobby }) => {
    // Storyteller context
    ...
    try {
      ...
      await playerPage.waitForURL('**/realms/botct/protocol/openid-connect/auth**');
