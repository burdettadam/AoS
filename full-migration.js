...
for (const scriptDir of scriptDirs) {
  ...
  try {
    ...
    const scriptName = townData.name || scriptId.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
    
    console.log(`📂 Processing ${scriptName} (${characters.length} characters)`);
    ...
    for (const character of characters) {
      ...
      const normalizedCharacter = {
        id: character.id,
        name: character.name || character.id.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
        category: character.category?.toLowerCase() || 'townsfolk',
        edition: Array.isArray(character.edition) ? character.edition : [scriptId],
        ability_summary: character.ability_summary || character.abilitySummary || character.ability || '',
        ability_description: character.ability_description || character.ability || character.ability_summary || '',
        first_night_reminder: character.first_night_reminder || character.firstNightAction || character.first_night_action || '',
        other_night_reminder: character.other_night_reminder || character.otherNightsAction || character.other_nights_action || '',
        setup: character.setup || false,
        tokens_used: character.tokens_used || character.tokensUsed || [],
        tags: character.tags || [],
        wiki_url: character.wiki_url || character.wikiUrl || `https://wiki.bloodontheclocktower.com/${character.name?.replace(/\s+/g, '_')}`,
      ...
      const isMainEdition = ['trouble-brewing', 'bad-moon-rising', 'sects-and-violets'].includes(scriptId);
    ...
    const scriptMetadata = {
      id: scriptId,
      name: scriptName,
      description: townData.description || townData.summary || `${scriptName} script for Blood on the Clocktower.`,
      ...
      playerCount: {
        ...
        optimal: townData.optimal_players || townData.player_count?.optimal || "7-10"
      },
