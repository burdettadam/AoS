...
async function main() {
  ...
  if (engine.getGame(gameId)!.phase !== GamePhase.SETUP) throw new Error('not in setup');
  ...
  if (g1.phase !== GamePhase.NIGHT || g1.day !== 1) throw new Error('bad phase after setup');

  const adv1 = await engine.advancePhase(gameId, storytellerSeatId);
  if (!adv1.ok || engine.getGame(gameId)!.phase !== GamePhase.DAY) throw new Error('advance to day failed');

  const adv2 = await engine.advancePhase(gameId, storytellerSeatId);
  if (!adv2.ok || engine.getGame(gameId)!.phase !== GamePhase.NOMINATION) throw new Error('advance to nomination failed');
  ...
  if (!adv3.ok || engine.getGame(gameId)!.phase !== GamePhase.VOTE) throw new Error('advance to vote failed');
  ...
  if (!('executed' in fin) || !fin.executed) throw new Error('execution did not occur');

  const adv4 = await engine.advancePhase(gameId, storytellerSeatId);
  if (!adv4.ok || engine.getGame(gameId)!.phase !== GamePhase.EXECUTION) throw new Error('advance to execution failed');
  const adv5 = await engine.advancePhase(gameId, storytellerSeatId);
  if (!adv5.ok || engine.getGame(gameId)!.phase !== GamePhase.NIGHT || engine.getGame(gameId)!.day !== 2) throw new Error('advance to night failed');
