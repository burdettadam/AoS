import { ScriptLoader, LoadedScript, Character } from '@botc/shared';
...
async function demonstrateScriptLoading() {
  ...
  try {
    ...
    console.log(`✅ Loaded "${troubleBrewing.name}" with ${troubleBrewing.characters.length} characters`);
    
    // Demonstrate type-safe character access
    const washerwoman = troubleBrewing.characters.find(char => char.id === 'washerwoman');
    if (washerwoman) {
      console.log(`\n🔍 Example Character: ${washerwoman.name}`);
      console.log(`   Team: ${washerwoman.team}`);
      console.log(`   Ability: ${washerwoman.ability}`);
      console.log(`   Reminders: ${washerwoman.reminders?.join(', ') || 'None'}`);
    }

    // Load a custom script
    console.log('\n📜 Loading custom script...');
    const customScript = await scriptLoader.loadScript('custom-scripts/catfishing');
    
    console.log(`✅ Loaded "${customScript.name}" with ${customScript.characters.length} characters`);
    if (customScript.meta) {
      console.log(`   Author: ${customScript.meta.author || 'Unknown'}`);
      console.log(`   Players: ${customScript.meta.playerCount?.min}-${customScript.meta.playerCount?.max}`);
      console.log(`   Complexity: ${customScript.meta.complexity || 'Unknown'}`);
    }

    // Demonstrate character filtering by team
    const demons = customScript.characters.filter(char => char.team === 'demon');
    ...
    allScripts.forEach(script => {
      ...
      console.log(`   • ${script.name} (${characterCount} characters, teams: ${teams.join(', ')})`);
    });

    // Demonstrate script statistics
    console.log('\n📊 Script Statistics:');
    const totalCharacters = allScripts.reduce((total, script) => total + script.characters.length, 0);
    const teamCounts = allScripts.reduce((counts, script) => {
...
function demonstrateCharacterValidation() {
  ...
  const legacyCharacterData = {
    ...
    ability_summary: 'Each night, you learn how many of your 2 alive neighbors are evil.',
    first_night_action: 'The Empath learns how many of their 2 alive neighbors are evil.',
    other_nights_action: 'The Empath learns how many of their 2 alive neighbors are evil.',
    day_action: null,
  ...
  console.log('📝 Legacy JSON format:', JSON.stringify(legacyCharacterData, null, 2));
...
export { demonstrateScriptLoading, demonstrateCharacterValidation };
