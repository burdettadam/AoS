...
export class GameEngine {
  ...
  async createGame(
    scriptId: string = "trouble-brewing",
  ...
  setGameName(
    ...
    if (game.phase !== GamePhase.LOBBY)
      return { ok: false, error: "Cannot rename after lobby" };
    if (!game.storytellerSeatId || game.storytellerSeatId !== storytellerSeatId)
      return { ok: false, error: "Only storyteller can set name" };
  ...
  async addPlayer(
    ...
    if (!isNPC) {
      const existing = game.seats.find(
        (s) => !s.isNPC && s.playerId === playerId,
      );
      if (existing) {
        logger.info(
          `Player ${playerId} already in game ${gameId} as seat ${existing.id}, reusing seat`,
        );
  ...
  removePlayer(
    ...
    if (!game) return { ok: false, error: "Game not found" };
    if (game.phase !== GamePhase.LOBBY)
      return { ok: false, error: "Can only leave during lobby" };
    ...
    if (wasStoryteller) {
      ...
      for (const s of game.seats)
        (s as any).isStoryteller = nextHuman ? s.id === nextHuman.id : false;
    }
    game.updatedAt = new Date();
    this.addEvent(gameId, {
      id: randomUUID(),
  ...
  proposeScript(
    ...
    if (active) {
      if (!proposal) {
        ...
        proposal.proposers = Array.isArray(proposal.proposers)
          ? proposal.proposers
          : [proposal.proposedBy].filter(Boolean);
      ...
      proposal.proposers = Array.isArray(proposal.proposers)
        ? proposal.proposers
        : [proposal.proposedBy].filter(Boolean);
      proposal.proposers = proposal.proposers.filter(
        (seat: SeatId) => seat !== proposer,
      );
      if (proposal.votes) delete proposal.votes[proposer as any];
      if (proposal.difficultyVotes)
        delete proposal.difficultyVotes[proposer as any];
    }
  ...
  private pruneProposalIfEmpty(game: GameState, proposal: any) {
    ...
    const difficultyCount = proposal?.difficultyVotes
      ? Object.keys(proposal.difficultyVotes).length
      : 0;
    const proposerCount = Array.isArray(proposal?.proposers)
      ? proposal.proposers.length
      : proposal?.proposedBy
        ? 1
        : 0;
  ...
  voteScriptDifficulty(
    ...
    const game = this.games.get(gameId);
    if (!game || game.phase !== GamePhase.LOBBY) return false;
    const proposal = (game as any).scriptProposals.find(
  ...
  setStoryteller(
    ...
    const game = this.games.get(gameId);
    if (!game || game.phase !== GamePhase.LOBBY)
      return { ok: false, error: "Can only set storyteller in lobby" };
    if (game.storytellerSeatId !== setterSeat)
      return {
        ok: false,
        error: "Only current storyteller can assign storyteller",
      };
    if (!game.seats.some((s) => s.id === targetSeat))
  ...
  claimRole(gameId: GameId, seatId: SeatId, roleId: string): boolean {
    ...
    const pool =
      (game as any).selectedRoles && (game as any).selectedRoles.length > 0
        ? ((game as any).selectedRoles as string[])
        : undefined;
    ...
    if (
      Object.values(
        (game as any).roleClaims as Record<string, string>,
      ).includes(roleId)
    )
      return false;
  ...
  async claimRandomRole(
    ...
    if ((game as any).roleClaims) {
      for (const rid of Object.values(
        (game as any).roleClaims as Record<string, string>,
      ))
        pool.delete(rid as any);
    }
    const remaining = Array.from(pool);
    if (remaining.length === 0) return null;
    const pick: string = remaining[
  ...
  async enterSetup(
    ...
    const game = this.games.get(gameId);
    if (!game) return { ok: false, error: "Game not found" };
    if (!this.isStoryteller(game, storytellerSeatId))
      return { ok: false, error: "Only storyteller can enter setup" };
    if (game.phase !== GamePhase.LOBBY)
      return { ok: false, error: "Can only enter setup from lobby" };
  ...
  async selectSetupCharacters(
    ...
    if (!game) return { ok: false, error: "Game not found" };
    if (!this.isStoryteller(game, storytellerSeatId))
      return { ok: false, error: "Only storyteller can select characters" };
    if (game.phase !== GamePhase.SETUP)
      return { ok: false, error: "Can only select characters during setup" };

    const script = await this.scriptLoader.loadScript(game.scriptId);
    if (!script) return { ok: false, error: "Failed to load script" };

    const result = this.setupManager.selectCharacters(
      game,
      script,
      characterIds,
      storytellerSeatId,
  ...
  async validateSetup(
    ...
    if (!game) return { ok: false, error: "Game not found" };
    if (!this.isStoryteller(game, storytellerSeatId))
      return { ok: false, error: "Only storyteller can validate setup" };
    if (game.phase !== GamePhase.SETUP)
      return { ok: false, error: "Can only validate during setup" };
  ...
  async completeSetup(
    ...
    if (!game) return { ok: false, error: "Game not found" };
    if (!this.isStoryteller(game, storytellerSeatId))
      return { ok: false, error: "Only storyteller can complete setup" };
    if (game.phase !== GamePhase.SETUP)
      return { ok: false, error: "Can only complete setup from setup phase" };
    ...
    logger.info(
      `Completed setup and started game ${gameId} with ${game.seats.length} players`,
    );
    return { ok: true };
  }
  ...
  async advancePhase(
    ...
    if (!game) return { ok: false, error: "Game not found" };
    if (!this.isStoryteller(game, storytellerSeatId))
      return { ok: false, error: "Only storyteller can advance phase" };
  ...
  async setPhase(
    ...
    if (!game) return { ok: false, error: "Game not found" };
    if (!this.isStoryteller(game, storytellerSeatId))
      return { ok: false, error: "Only storyteller can set phase" };
    if (game.phase === target) return { ok: true };
    if (target === GamePhase.LOBBY)
      return { ok: false, error: "Cannot return to lobby" };

    const next = this.getNextPhase(game.phase);
    if (target !== next && target !== GamePhase.END) {
      return {
        ok: false,
        error: `Invalid phase transition ${game.phase} -> ${target}`,
      };
    }
  ...
  getPublicGames(): GameState[] {
    return Array.from(this.games.values()).filter(
      (game) => game.isPublic && game.phase === GamePhase.LOBBY,
    );
  }
  ...
  validateGameState(game: GameState): string[] {
    ...
    if (
      game.storytellerSeatId &&
      !game.seats.some((s) => s.id === game.storytellerSeatId)
    ) {
    ...
    if (game.phase !== GamePhase.LOBBY) {
      ...
      if (unassigned.length > 0)
        issues.push(`unassigned roles for ${unassigned.length} seat(s)`);
    }
    return issues;
  }
  ...
  private validatePreconditions(
    ...
    if (target === GamePhase.SETUP) {
      if (game.phase !== GamePhase.LOBBY)
        return { ok: false, error: "Can only enter setup from lobby" };
      if (!game.storytellerSeatId)
        return {
          ok: false,
          error: "Storyteller must be assigned before setup",
        };
    }
    ...
    if (target === GamePhase.NIGHT) {
      if (game.phase === GamePhase.SETUP) {
        if (!game.setupState || !game.setupState.isValidated) {
          return {
            ok: false,
            error: "Setup must be validated before starting game",
          };
        }
        if (
          !game.setupState.characterPool ||
          game.setupState.characterPool.length === 0
        ) {
          return {
            ok: false,
            error: "Character pool must be created before starting game",
          };
        }
      }
    }
    ...
    if (target === GamePhase.NOMINATION) {
      if (game.phase !== GamePhase.DAY)
        return { ok: false, error: "Can only nominate during Day" };
      if ((game as any).currentVote && !(game as any).currentVote.finished)
        return {
          ok: false,
          error: "Cannot move to nomination while a vote is active",
        };
    }
    // From Nomination -> Vote: must have a current nomination
    if (target === GamePhase.VOTE) {
      if (
        !(game as any).currentNomination ||
        (game as any).currentNomination.closed
      )
        return { ok: false, error: "No open nomination to vote on" };
    }
    // From Vote -> Execution: must have a finished vote
    if (target === GamePhase.EXECUTION) {
      if (!(game as any).currentVote || !(game as any).currentVote.finished)
        return { ok: false, error: "No finished vote to execute" };
    }
  ...
  nominate(
    ...
    nominator: SeatId,
    nominee: SeatId,
  ): { ok: true; nominationId: string } | { ok: false; error: string } {
    const game = this.games.get(gameId);
    if (!game) return { ok: false, error: "Game not found" };
    if (game.phase !== GamePhase.NOMINATION && game.phase !== GamePhase.DAY)
      return { ok: false, error: "Not in nomination phase" };
    if (
      !game.seats.some((s) => s.id === nominator) ||
      !game.seats.some((s) => s.id === nominee)
    )
      return { ok: false, error: "Invalid seats" };
    if (!game.seats.find((s) => s.id === nominator)?.isAlive)
      return { ok: false, error: "Nominator is dead" };
    if (!game.seats.find((s) => s.id === nominee)?.isAlive)
      return { ok: false, error: "Nominee is dead" };
    // Only one open nomination at a time
    if (
      (game as any).currentNomination &&
      !(game as any).currentNomination.closed
    )
      return { ok: false, error: "Nomination already open" };
  ...
  startVote(
    ...
    if (!game) return { ok: false, error: "Game not found" };
    if (!this.isStoryteller(game, storytellerSeatId))
      return { ok: false, error: "Only storyteller can start vote" };
    if (game.phase !== GamePhase.VOTE)
      return { ok: false, error: "Not in vote phase" };
    if (
      !(game as any).currentNomination ||
      (game as any).currentNomination.closed
    )
      return { ok: false, error: "No open nomination" };
    if ((game as any).currentVote && !(game as any).currentVote.finished)
      return { ok: false, error: "Vote already in progress" };

    (game as any).currentNomination.closed = true as any;
    (game as any).currentVote = {
      nominationId: (game as any).currentNomination.id,
      startedAt: new Date(),
      votes: [],
      tally: { yes: 0, no: 0 },
      finished: false,
  ...
  castVote(
    ...
    if (game.phase !== GamePhase.VOTE)
      return { ok: false, error: "Not in vote phase" };
    if (!(game as any).currentVote || (game as any).currentVote.finished)
      return { ok: false, error: "No active vote" };
    const seat = game.seats.find((s) => s.id === voter);
    if (!seat || !seat.isAlive) return { ok: false, error: "Invalid voter" };

    // Prevent duplicate votes
    if ((game as any).currentVote.votes.some((v: any) => v.voter === voter))
      return { ok: false, error: "Already voted" };
    (game as any).currentVote.votes.push({
      voter,
      vote,
      timestamp: new Date(),
    } as any);
    if (vote) (game as any).currentVote.tally.yes += seat.votingPower;
    else (game as any).currentVote.tally.no += seat.votingPower;
    this.addEvent(gameId, {
      id: randomUUID(),
      gameId,
      type: "vote_cast",
  ...
  finishVote(
    ...
    | { ok: true; executed: boolean; nominee?: SeatId }
    | { ok: false; error: string } {
    const game = this.games.get(gameId);
    if (!game) return { ok: false, error: "Game not found" } as any;
    if (!this.isStoryteller(game, storytellerSeatId))
      return { ok: false, error: "Only storyteller can finish vote" } as any;
    if (game.phase !== GamePhase.VOTE)
      return { ok: false, error: "Not in vote phase" } as any;
    if (
      !(game as any).currentVote ||
      !(game as any).currentNomination ||
      (game as any).currentVote.finished
    )
      return { ok: false, error: "No active vote" } as any;

    (game as any).currentVote.finished = true as any;
    const executed =
      (game as any).currentVote.tally.yes > (game as any).currentVote.tally.no;
    if (executed) {
      const nomineeSeat = game.seats.find(
        (s) => s.id === (game as any).currentNomination!.nominee,
      );
      ...
      this.addEvent(gameId, {
        ...
        timestamp: new Date(),
        payload: {
          seatId: nomineeSeat?.id,
          tally: (game as any).currentVote.tally,
        },
      });
    ...
    return {
      ok: true,
      executed,
      nominee: (game as any).currentNomination.nominee,
    } as any;
  }
  ...
  async executeNightOrder(
    ...
    if (game.phase !== GamePhase.NIGHT)
      return { ok: false, error: "Not in night phase" };

    try {
      const { results, events } =
        await this.nightOrderProcessor.executeNightOrder(game, isFirstNight);
  ...
  async getNightOrderPreview(
    ...
    try {
      return await this.nightOrderProcessor.previewNightOrder(
        game,
        isFirstNight,
      );
    } catch (error) {
      logger.error(
        `Failed to get night order preview for game ${gameId}:`,
        error,
      );
      return null;
    }
  ...
  async getActiveCharacters(
    ...
    try {
      return await this.nightOrderProcessor.getActiveCharacters(
        game,
        isFirstNight,
      );
    } catch (error) {
      logger.error(
        `Failed to get active characters for game ${gameId}:`,
        error,
      );
      return null;
    }
  ...
  async getCharacterActions(
    ...
    try {
      return await this.scriptLoader.getCharacterActions(
        game.scriptId,
        characterId,
        phase,
      );
      ...
      logger.error(
        `Failed to get character actions for ${characterId} in game ${gameId}:`,
        error,
      );
      return null;
    }
  ...
  async generateValidationReport(gameId: GameId): Promise<string | null> {
    ...
    try {
      const validationResult = await this.scriptLoader.validateScript(
        game.scriptId,
      );
      return this.validationSystem.generateReport(validationResult);
    } catch (error) {
      logger.error(
        `Failed to generate validation report for game ${gameId}:`,
        error,
      );
      return null;
    }
  ...
  async testActionExecution(
    gameId: GameId,
  ): Promise<{ ok: true; tests: any[] } | { ok: false; error: string }> {
    const game = this.games.get(gameId);
    if (!game) return { ok: false, error: "Game not found" };
    ...
    try {
      ...
      for (const metaAction of metaActions) {
        try {
          ...
          const result = await this.actionSystem.executeMetaAction(
            metaAction,
            context,
            game,
          );
      ...
      for (const seat of game.seats) {
        if (seat.role && seat.isAlive) {
          const loadedScript = await this.scriptLoader.getLoadedScript(
            game.scriptId,
          );
          const character = loadedScript?.characters.find(
            (c) => c.id === seat.role,
          );
          ...
          if (character?.actions?.firstNight) {
            for (const action of character.actions.firstNight) {
              try {
                ...
                const result = await this.actionSystem.executeCharacterAction(
                  action,
                  context,
                  game,
                  character,
      ...
      logger.error(
        `Failed to test action execution for game ${gameId}:`,
        error,
      );
      return {
        ok: false,
