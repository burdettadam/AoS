...
export class ValidationSystem {
  ...
  validateScript(script: LoadedScript): ScriptValidationResult {
    ...
    for (const character of script.characters) {
      ...
      if (!charResult.isValid) {
        ...
        result.errors.push(
          ...charResult.errors.map((e) => `${character.id}: ${e}`),
        );
      }

      result.warnings.push(
        ...charResult.warnings.map((w) => `${character.id}: ${w}`),
      );
      result.missingConfigurations.push(
        ...charResult.missingConfigurations.map((m) => `${character.id}: ${m}`),
      );
    }
  ...
  private checkExpectedActions(
    ...
    if (character.firstNight && character.firstNight > 0) {
      if (
        !character.actions?.firstNight ||
        character.actions.firstNight.length === 0
      ) {
        result.missingActions.firstNight = true;
        result.missingConfigurations.push(
          "Missing firstNight actions array despite having firstNight order",
        );
      }
    }
    ...
    if (character.otherNights && character.otherNights > 0) {
      if (!character.actions?.night || character.actions.night.length === 0) {
        ...
        result.missingConfigurations.push(
          "Missing otherNights actions array despite having otherNights order",
        );
      }
    }
    ...
    if (dayKeywords.some((keyword) => abilityText.includes(keyword))) {
      const hasDayActions =
        character.actions?.day && character.actions.day.length > 0;
      if (!hasDayActions) {
        result.missingActions.day = true;
        result.warnings.push(
          `Ability suggests day action but no day actions configured (has actions: ${!!character.actions}, day length: ${character.actions?.day?.length || 0})`,
        );
      }
    }
  }
   ...
   */
  private validateCharacterActions(
    character: Character,
    result: CharacterValidationResult,
  ): void {
  ...
  private validateAction(
    ...
    const knownCharacterActions = Object.values(
      CharacterActionType,
    ) as string[];
    ...
    if (!allKnownActions.includes(actionType)) {
      result.missingConfigurations.push(
        `Action type '${actionType}' not implemented in engine`,
      );
    }
    ...
    if (phase.includes("night") && action.order === undefined) {
      result.warnings.push(
        `Action ${actionId} missing order for night sequencing`,
      );
    }
  }
   ...
   */
  private validateInformationSpec(
    info: any,
    actionId: string,
    result: CharacterValidationResult,
  ): void {
    const hasShowPlayers =
      info.showPlayersByTeam || info.showPlayers || info.showPlayer;
    const hasCustomMessage = info.customMessage;
    const hasBluffs = info.giveBluffs;

    if (!hasShowPlayers && !hasCustomMessage && !hasBluffs) {
      result.warnings.push(
        `Action ${actionId} has information spec but no delivery method specified`,
      );
    }

    if (
      info.customMessage &&
      info.customMessage.includes("[COUNT]") &&
      !actionId.includes("count")
    ) {
      result.warnings.push(
        `Action ${actionId} uses [COUNT] placeholder but action name doesn't suggest counting`,
      );
    }
  }
   ...
   */
  private checkActionCompleteness(
    character: Character,
    result: CharacterValidationResult,
  ): void {
    ...
    if (character.goal) {
      ...
      if (!hasMatchingAction) {
        result.warnings.push(
          `Goal specifies action '${goalAction}' but no matching action found`,
        );
      }
    }
    ...
    if (character.actions && !character.goal) {
      const hasComplexActions = Object.values(character.actions).some(
        (actions) => actions && actions.length > 0,
      );

      if (hasComplexActions) {
        result.recommendations.push(
          "Character has actions but no goal specified - consider adding goal metadata",
        );
      }
    }
  }
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      if (typeof entry === "string") {
        ...
        if (!characterIds.has(entry)) {
          result.errors.push(
            `${phase}[${i}]: Character '${entry}' not found in script`,
          );
          result.isValid = false;
        }
      } else {
  ...
  private validateMetaAction(
    ...
    if (!action.targets || action.targets.length === 0) {
      result.warnings.push(
        `${context}: Meta action ${action.id} has no targets`,
      );
    }
    ...
    if (!metaActionType) {
      result.errors.push(
        `${context}: Meta action ${action.id} missing action type`,
      );
      result.isValid = false;
      return;
    }
    ...
    if (!knownMetaActions.includes(metaActionType)) {
      result.missingConfigurations.push(
        `${context}: Meta action '${metaActionType}' not implemented in engine`,
      );
    }
  }
  ...
  private validateMetaActions(
    ...
    if (hasMinions || hasDemons) {
      ...
      const hasMinionsInfo = firstNightActions.some(
        (entry) =>
          typeof entry === "object" && entry.action === "showTeamToMinions",
      );
      const hasDemonInfo = firstNightActions.some(
        (entry) =>
          typeof entry === "object" &&
          entry.action === "showTeamAndBluffsToDemon",
      );

      if (hasMinions && !hasMinionsInfo) {
        result.missingMetaActions.push(
          "showTeamToMinions - required when minions are present",
        );
      }

      if (hasDemons && !hasDemonInfo) {
        result.missingMetaActions.push(
          "showTeamAndBluffsToDemon - required when demons are present",
        );
      }
    }
  }
   ...
   */
  private generateScriptRecommendations(
    script: LoadedScript,
    result: ScriptValidationResult,
  ): void {
    const missingActionCharacters = result.characterResults.filter(
      (cr) => Object.keys(cr.missingActions).length > 0,
    );

    if (missingActionCharacters.length > 0) {
      result.recommendations.push(
        `${missingActionCharacters.length} characters missing action configurations: ${missingActionCharacters
          .map((cr) => cr.characterId)
          .join(", ")}`,
      );
    }
    ...
    if (unimplementedActions.length > 0) {
      result.recommendations.push(
        `Consider implementing these action types in the engine: ${[
          ...new Set(unimplementedActions.map((ua) => ua.split("'")[1])),
        ].join(", ")}`,
      );
    }
  }
  ...
  generateReport(result: ScriptValidationResult): string {
    ...
    if (result.missingConfigurations.length > 0) {
      ...
      result.missingConfigurations.forEach((missing) =>
        lines.push(`  - ${missing}`),
      );
      lines.push("");
    }
    ...
    // Character summary
    const validCharacters = result.characterResults.filter(
      (cr) => cr.isValid,
    ).length;
    const totalCharacters = result.characterResults.length;
    lines.push(
      `CHARACTER SUMMARY: ${validCharacters}/${totalCharacters} valid`,
    );

    const incompleteCharacters = result.characterResults.filter(
      (cr) => !cr.isValid,
    );
    if (incompleteCharacters.length > 0) {
      ...
      incompleteCharacters.forEach((cr) => {
        lines.push(
          `  - ${cr.characterId}: ${cr.errors.length} errors, ${cr.warnings.length} warnings`,
        );
      });
    }
