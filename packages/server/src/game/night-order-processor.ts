...
export class NightOrderProcessor {
  ...
  async executeNightOrder(
    ...
    logger.info(
      `Executing ${isFirstNight ? "first" : "other"} night order for game ${game.id}`,
    );

    const nightOrder = await this.scriptLoader.getNightOrder(
      game.scriptId,
      isFirstNight,
    );
    ...
    for (let i = 0; i < nightOrder.length; i++) {
      ...
      logger.debug(
        `Processing night order entry ${i}: ${typeof entry === "string" ? entry : entry.id}`,
      );

      try {
        if (typeof entry === "string") {
          // Character action
          const result = await this.executeCharacterNightAction(
            entry,
            game,
            context,
            isFirstNight,
  ...
  private async executeCharacterNightAction(
    ...
    return this.actionSystem.executeCharacterAction(
      action,
      actionContext,
      game,
      character,
    ...
    for (const entry of nightOrder) {
      if (typeof entry === "string") {
        ...
        const seat = game.seats.find(
          (s) => s.role === characterId && s.isAlive,
        );

        if (seat) {
          const actions = await this.scriptLoader.getCharacterActions(
            game.scriptId,
            characterId,
            phase,
          );
