import { Character, LoadedScript } from "@ashes-of-salem/shared";
...
function seedToTarget(
  ...
  (["townsfolk", "outsiders", "minions", "demons"] as (keyof Counts)[]).forEach(
    (teamKey) => {
      const singular = COUNTKEY_TO_TEAM[teamKey];
      while ((counts[teamKey] || 0) < (targetCounts[teamKey] || 0)) {
        const candidate = pool.find(
          (id) => teamOf(id, characters) === singular && !result.has(id),
        );
        if (!candidate) break;
...
export function resolveLineup({
  ...
  const pool: string[] =
    (script.meta?.characterList as any) ||
    ((script as any).characters?.map((c: Character) => c.id) as string[]) ||
    [];
