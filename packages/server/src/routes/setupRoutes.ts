...
export default async function setupRoutes(
  ...
  fastify.post<{ Params: SetupParams; Body: EnterSetupBody }>(
    "/:gameId/setup/enter",
    async (
      request: FastifyRequest<{ Params: SetupParams; Body: EnterSetupBody }>,
      reply: FastifyReply,
    ) => {
      try {
        ...
        if (!storytellerSeatId) {
          return reply
            .status(400)
            .send({ error: "storytellerSeatId is required" });
        }
    ...
    async (
      ...
      try {
        ...
        if (!Array.isArray(characterIds)) {
          return reply
            .status(400)
            .send({ error: "characterIds must be an array" });
        }

        const result = await gameEngine.selectSetupCharacters(
          gameId,
          storytellerSeatId,
          characterIds,
        );
  ...
  fastify.post<{ Params: SetupParams; Body: ValidateSetupBody }>(
    ...
    async (
      request: FastifyRequest<{ Params: SetupParams; Body: ValidateSetupBody }>,
      reply: FastifyReply,
    ) => {
      try {
        const { gameId } = request.params;
        ...
        const result = await gameEngine.validateSetup(
          gameId,
          storytellerSeatId,
        );
  ...
  fastify.post<{ Params: SetupParams; Body: CompleteSetupBody }>(
    ...
    async (
      request: FastifyRequest<{ Params: SetupParams; Body: CompleteSetupBody }>,
      reply: FastifyReply,
    ) => {
      try {
        const { gameId } = request.params;
        ...
        const result = await gameEngine.completeSetup(
          gameId,
          storytellerSeatId,
        );
  ...
  fastify.get<{ Params: SetupParams }>(
    "/:gameId/setup",
    async (
      request: FastifyRequest<{ Params: SetupParams }>,
      reply: FastifyReply,
    ) => {
      try {
        const { gameId } = request.params;
        ...
        try {
          ...
          // Failed to load script, continue with null
        }

        const playerCount = game.seats.filter(
          (s: any) => !s.isStoryteller,
        ).length;
        ...
        const expectedDistribution = { ...baseDistribution } as Record<
          string,
          number
        >;
        ...
        for (const mod of mods) {
          switch (mod.type) {
            case "add_outsiders":
              expectedDistribution.outsiders += mod.count;
              break;
            case "remove_townsfolk":
              expectedDistribution.townsfolk -= mod.count;
              break;
            case "add_minions":
              expectedDistribution.minions += mod.count;
              break;
        ...
        const availableCharacters = Array.isArray(script?.roles)
          ? script.roles.map((r: any) => ({
              id: r.id,
              name: r.name,
              team:
                r.type === RoleType.TOWNSFOLK
                  ? "townsfolk"
                  : r.type === RoleType.OUTSIDER
                    ? "outsiders"
                    : r.type === RoleType.MINION
                      ? "minions"
                      : r.type === RoleType.DEMON
                        ? "demons"
                        : "unknown",
