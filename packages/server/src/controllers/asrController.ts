...
export class ASRController {
  ...
  static async streamAudio(
    request: FastifyRequest<{ Body: StreamAudioBody }>,
    reply: FastifyReply,
  ) {
    try {
      ...
      return {
        ...
        eventsExtracted: result.transcript
          ? asrService.extractGameEvents(result.transcript, seatId).length
          : 0,
      };
    } catch (error) {
   ...
   */
  static async summarizePhase(
    request: FastifyRequest<{ Body: SummarizePhaseBody }>,
    reply: FastifyReply,
  ) {
    try {
