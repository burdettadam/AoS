...
export class KeycloakAuth {
  ...
  // Legacy method retained for compatibility; prefer `optional` or a future `required` method
  handleAuthError = async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const decoded = await new Promise<KeycloakToken>((resolve, reject) => {
        jwt.verify(
          ...
          (err, decoded) => {
            if (err) reject(err);
            else resolve(decoded as KeycloakToken);
          },
        );
      });
  ...
  public optional = async (request: FastifyRequest, _reply: FastifyReply) => {
    try {
      const authHeader = request.headers.authorization;
