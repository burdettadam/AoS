...
class ScriptCache {
  ...
  async initialize(): Promise<void> {
    ...
    logger.info(
      `✅ Script cache initialized in ${endTime - startTime}ms - ${this.cache.size} scripts loaded`,
    );
    ...
    try {
      ...
      const loadPromises = scriptPaths.map(async ({ id, path: scriptPath }) => {
        try {
          ...
          const scriptData = await fs
            .readFile(fullScriptPath, "utf-8")
            .then(JSON.parse);
          ...
          if (characters.length === 0) {
            logger.warn(
              `⚠️  No characters found for script ${scriptData.name || id} - check character database`,
            );
          }
          ...
          const script = {
            ...
            meta: this.createScriptMetadataFromNewFormat(
              scriptData,
              scriptData.id || id,
            ),
            modifiers: Array.isArray(scriptData.modifiers)
              ? scriptData.modifiers
              : undefined,
          } as LoadedScript;
          ...
          logger.info(
            `📜 Loaded script: ${scriptData.name || id} (${characters.length} characters)`,
          );
        } catch (error) {
          logger.warn(
            `⚠️  Failed to load script ${id}:`,
  ...
  private createScriptMetadata(
    ...
    const fallbackImages: Record<string, string> = {
      "trouble-brewing":
        "https://wiki.bloodontheclocktower.com/File:Logo_trouble_brewing.png",
      "bad-moon-rising":
        "https://wiki.bloodontheclocktower.com/File:Logo_bad_moon_rising.png",
      "sects-and-violets":
        "https://wiki.bloodontheclocktower.com/File:Logo_sects_and_violets.png",
    };

    const meta: ScriptMetadata = {
      id: scriptId,
      name:
        townData?.name || this.scriptsMetadata.get(scriptId)?.name || scriptId,
      author: townData?.author,
      description: townData?.description,
      version: townData?.version || "1.0.0",
      tags: townData?.tags,
      playerCount:
        townData?.player_count || this.getDefaultPlayerCount(scriptId),
  ...
  private createScriptMetadataFromNewFormat(
    scriptData: any,
    scriptId: string,
  ): ScriptMetadata {
    const meta: ScriptMetadata = {
      ...
      description:
        ...
        `${scriptData.name || scriptId} script for Blood on the Clocktower.`,
      version: scriptData.version || "1.0.0",
      tags: scriptData.tags || [],
      playerCount:
        scriptData.playerCount || this.getDefaultPlayerCount(scriptId),
  ...
  private getDefaultPlayerCount(scriptId: string): {
    min: number;
    max: number;
    optimal?: string;
  } {
    const standardCounts: Record<
      string,
      { min: number; max: number; optimal?: string }
    > = {
  ...
  getScriptsList(): Array<{ id: string; name: string }> {
    return Array.from(this.scriptsMetadata.values()).map(({ id, name }) => ({
      id,
      name,
    }));
  }
  ...
  getStats(): { scriptCount: number; totalCharacters: number } {
    ...
    return {
      ...
      totalCharacters: scripts.reduce(
