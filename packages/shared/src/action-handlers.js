...
exports.META_ACTION_HANDLERS = exports.CHARACTER_ACTION_HANDLERS = void 0;
...
function handleChoosePlayer(action, context, game, actingSeat) {
    ...
    const targets = context.targets ? context.targets.map(id => game.seats.find(s => s.id === id)).filter(Boolean) : [];
    if (action.effects) {
        action_registry_1.EffectProcessor.applyEffects(action, targets, actingSeat, game);
    }
...
function handleEnforceVotingRestriction(action, context, game, actingSeat) {
    ...
    action_registry_1.EffectProcessor.applyEffects(action, [], actingSeat, game);
    return {
        actionId: action.id,
        success: true,
        information: {
...
function handleShowTeamToMinions(action, context, game) {
    ...
    const evilTeam = game.seats.filter(seat => seat.role && (isMinion(seat) || isDemon(seat))).map(seat => getSeatDisplayName(seat));
...
exports.CHARACTER_ACTION_HANDLERS = {
    [game_definitions_1.CharacterActionType.LEARN_EVIL_PAIRS_COUNT]: handleLearnEvilPairsCount,
    [game_definitions_1.CharacterActionType.LEARN_EVIL_NEIGHBOR_COUNT]: handleLearnEvilNeighborCount,
    [game_definitions_1.CharacterActionType.LEARN_PLAYER_INFO]: handleLearnPlayerInfo,
    [game_definitions_1.CharacterActionType.CHOOSE_PLAYER]: handleChoosePlayer,
    [game_definitions_1.CharacterActionType.CHOOSE_MASTER]: handleChooseMaster,
    [game_definitions_1.CharacterActionType.KILL_PLAYER]: handleKillPlayer,
    [game_definitions_1.CharacterActionType.PROTECT_PLAYER]: handleProtectPlayer,
    [game_definitions_1.CharacterActionType.POISON_PLAYER]: handlePoisonPlayer,
    [game_definitions_1.CharacterActionType.ENFORCE_VOTING_RESTRICTION]: handleEnforceVotingRestriction,
};
exports.META_ACTION_HANDLERS = {
    [game_definitions_1.MetaActionType.SHOW_TEAM_TO_MINIONS]: handleShowTeamToMinions,
    [game_definitions_1.MetaActionType.SHOW_TEAM_AND_BLUFFS_TO_DEMON]: handleShowTeamAndBluffsToDemon,
};
