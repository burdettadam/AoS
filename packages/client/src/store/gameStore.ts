...
import { GameId, GameState, LoadedScript, WSMessage } from "@botc/shared";
...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  seatId: (() => {
    try {
      return localStorage.getItem("botc-seat-id");
    } catch {
      return null;
    }
  })(),
  isStoryteller: (() => {
    try {
      const v = localStorage.getItem("botc-is-storyteller");
    ...
    websocket.onmessage = (event) => {
      try {
        ...
        if (message.type === "event") {
          ...
          const snapshot = (message.event.payload as any)?.gameState as
            | GameState
            | undefined;
          if (snapshot) {
            ...
            set({
              ...
              isStoryteller: !!seatId && snapshot.storytellerSeatId === seatId,
            });
            return;
          }
          ...
          switch (message.event.type) {
            ...
            case "storyteller_changed": {
              ...
              set((state) => {
                ...
                const updated = {
                  ...state.currentGame,
                  storytellerSeatId: payload.storytellerSeatId,
                } as GameState;
                const seatId = state.seatId;
                return {
                  currentGame: updated,
                  isStoryteller:
                    !!seatId && payload.storytellerSeatId === seatId,
                };
              });
              break;
            }
  ...
  setCurrentGame: (game: GameState) => {
    ...
    set({
      currentGame: game,
      isStoryteller: !!seatId && game.storytellerSeatId === seatId,
    });
  },
  ...
  setSeat: (seatId: string, isStoryteller?: boolean) => {
    try {
      ...
      if (typeof isStoryteller === "boolean")
        localStorage.setItem(
          "botc-is-storyteller",
          JSON.stringify(!!isStoryteller),
        );
    } catch {
  ...
  selectCharacters: async (characterIds: string[]): Promise<boolean> => {
    ...
    try {
      const response = await SetupApi.selectCharacters(
        state.gameId,
        state.seatId,
        characterIds,
      );
    ...
    try {
      ...
      if (response.success) {
        ...
        set({
          setupState: {
            ...(get().setupState || {}),
            validation: { isValid: valid, issues: details },
          },
        });
      } else {
        ...
        set({
          setupState: {
            ...(get().setupState || {}),
            validation: { isValid: false, issues: response.details || [] },
          },
        });
      }
    } catch {
  ...
  validateSetup: async (): Promise<boolean> => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.details) {
          set({
            setupError: `${response.error}: ${response.details.join(", ")}`,
          });
        }
      }
  ...
  loadSetupState: async (): Promise<void> => {
    ...
    try {
      ...
      if (response) {
        set((s) => ({
          ...
          currentGame: s.currentGame
            ? { ...s.currentGame, phase: response.phase as any }
            : s.currentGame,
        }));
      } else {
      ...
      set({
        setupError:
          error instanceof Error ? error.message : "Failed to load setup state",
      });
    }
  },
  ...
  loadScripts: async () => {
    ...
    try {
      ...
      const scriptsPromises = scriptsList.map(
      ...
      console.log(
        `⚡ Loaded ${scripts.length} scripts in ${endTime - startTime}ms (cache-optimized)`,
      );
    } catch (error) {
  ...
  loadScript: async (scriptId: string): Promise<LoadedScript | null> => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to load script ${scriptId}: ${response.statusText}`,
        );
      }
