  ...
  const goals = {
    townsfolk: {
      "Learns that one of two players is a particular Townsfolk": {
        ...
        description:
          "Confirms a specific Townsfolk character is in play and identifies one good player",
        targets: ["townsfolk"],
        effect:
          "Provides definitive proof of character presence and player alignment",
        frequency: "once",
      },
      ...
      "Learns that one of two players is a particular Minion": {
        ...
        description:
          "Identifies a specific Minion character in play and narrows down evil players",
        targets: ["minions"],
        effect:
          "Provides definitive proof of Minion presence and creates execution targets",
        frequency: "once",
      },
    },
    outsider: {
      default: {
        ...
        targets: ["self"],
        effect:
          "Requires special win conditions beyond standard good team victory",
        frequency: "game",
      },
    },
    minion: {
      default: {
        ...
        targets: ["demons"],
        effect:
          "Provides various abilities to assist the Demon in achieving victory",
        frequency: "game",
      },
    },
    demon: {
      default: {
        ...
        effect:
          "Removes players from the game through various killing mechanisms",
        frequency: "nightly",
      },
    },
  };
...
function convertTipsActions(tips, characterId) {
  if (!tips || tips.trim() === "")
    return { day: [], nominations: [], voting: [], execution: [] };
  ...
  if (
    ...
    actions.day.push({
      ...
      information: {
        customMessage:
          "Strategic information sharing based on character ability",
      },
      order: order++,
    });
  }
  ...
  if (tips.includes("nominate") || tips.includes("execute")) {
    actions.nominations.push({
      ...
      information: {
        customMessage:
          "Uses character knowledge to make informed nomination decisions",
      },
      order: 1,
    });
  }
  ...
  if (tips.includes("vote")) {
    actions.voting.push({
      ...
      information: {
        customMessage:
          "Uses character knowledge to make informed voting decisions",
      },
      order: 1,
    });
  }
  ...
  if (
    tips.includes("survive") ||
    tips.includes("avoid") ||
    tips.includes("defend")
  ) {
...
function convertCharacter(characterPath) {
  try {
    ...
    // Add actions field
    character.actions = {
      firstNight: convertFirstNightActions(
        character.firstNightDescription,
        character.id,
      ),
      otherNights: [],
...
const files = fs
  ...
  .filter(
    (file) =>
      !["washerwoman.json", "investigator.json", "imp.json"].includes(file),
