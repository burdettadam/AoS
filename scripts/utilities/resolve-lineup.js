  ...
  for (const teamKey of ["townsfolk", "outsiders", "minions", "demons"]) {
    ...
    while ((counts[teamKey] || 0) < (targetCounts[teamKey] || 0)) {
      const candidate = pool.find(
        (id) => teamOf(id, charsMap) === singular && !result.has(id),
      );
      if (!candidate) break;
...
function applyMutuallyExclusive(selection, rules) {
  ...
  for (const r of rules) {
    ...
    while (present.length > 1) {
      ...
      notes.push(
        `mutuallyExclusive: removed '${drop}' (${characters.join(", ")})`,
      );
  ...
  for (const r of rules) {
    ...
    if (!anyPresent) {
      ...
      if (pick && !selection.includes(pick)) {
        ...
        added.add(pick);
        notes.push(
          `atLeastOneOf: added '${pick}' from [${r.characters.join(", ")}]`,
        );
      }
    }
  }
...
function applyAdjustCounts(selection, rules, pool, charsMap, protectedSet) {
  ...
  for (const r of rules) {
    ...
    notes.push(
      `adjustCounts: ${r.whenCharacter} => ${JSON.stringify(r.delta)}`,
    );
  }
  if (Object.values(totalDelta).every((v) => v === 0)) {
    return {
      selection,
      notes,
      targetCounts: summarizeCounts(selection, charsMap),
    };
  }
  ...
  for (const teamKey of ["townsfolk", "outsiders", "minions", "demons"]) {
    const singular = COUNTKEY_TO_TEAM[teamKey];
    while (
      (summarizeCounts(selection, charsMap)[teamKey] || 0) <
      (target[teamKey] || 0)
    ) {
      const candidate = pool.find(
        (id) => teamOf(id, charsMap) === singular && !selection.includes(id),
      );
      if (!candidate) {
        notes.push(`adjustCounts: unable to fill +${team}`);
        break;
      }
      selection.push(candidate);
      notes.push(
        `adjustCounts: added '${candidate}' to meet ${teamKey} target`,
      );
    }
  }
  ...
  for (const teamKey of ["townsfolk", "outsiders", "minions", "demons"]) {
    ...
    while (
      ...
      const idx = [...selection]
        .reverse()
        .findIndex(
          (id) => teamOf(id, charsMap) === singular && !protectedSet.has(id),
        );
      if (idx === -1) {
        notes.push(`adjustCounts: unable to reduce ${team}`);
        break;
      }
      const realIdx = selection.length - 1 - idx;
      const removed = selection.splice(realIdx, 1)[0];
      notes.push(
        `adjustCounts: removed '${removed}' to meet ${teamKey} target`,
      );
    }
  }
...
async function resolve({ scriptId, playerCount, selected }) {
  ...
  selection = res.selection;
  const protectedSet = new Set(res.added);
  applied.push(...res.notes);

  // atLeastOneOf
  res = applyAtLeastOneOf(selection, byType("atLeastOneOf"), pool);
  selection = res.selection;
  for (const a of res.added) protectedSet.add(a);
  applied.push(...res.notes);

  // adjustCounts
  res = applyAdjustCounts(
    selection,
    byType("adjustCounts"),
    pool,
    charsMap,
...
if (require.main === module) {
  ...
  const selected = selectArg
    ? selectArg
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean)
    : [];
  resolve({ scriptId, playerCount: players, selected })
    .then((out) => {
